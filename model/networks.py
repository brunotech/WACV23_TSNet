...
def init_weights(net, init_type='normal', init_gain=0.02):
    ...
    def init_func(m):  # define the initialization function
        ...
        # nhm added for adain layer
        if classname.find('adain_layer') != -1:
            init.constant_(m.style.bias.data[:m.style.out_features//2], 1.0)
            init.constant_(m.style.bias.data[m.style.out_features//2:], 0.0)
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        if gan_mode == 'lsgan':
            ...
            self.loss = nn.BCEWithLogitsLoss()  # it has included sigmoid in loss fun.
...
class VideoDiscriminator(nn.Module):
    def __init__(self, out_nc=16):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(ndf * 32, out_nc, 4, 1, 0, bias=False),
        )
        init_net(self.main, init_type='normal', init_gain=0.02)
...
if __name__  == "__main__":
    ...
    input_tensor = torch.rand(size=(4, 3, 256, 256)).cuda()

...
def read_keypoints(opt, json_input, size, basic_point_only=False, remove_face_labels=False,
    ...
    for keypoint_dict in keypoint_dicts:
        ...
        if ref_pts is not None and not opt.remove_face_labels:
...
def interp_points(x, y):
    ...
    # start_dist = np.linalg.norm(start_point - fit_start_point)
    # end_point = np.array([x[-1], y[-1]])
    # fit_end_point = np.array([curve_x[-1], curve_y[-1]])
    # end_dist = np.linalg.norm(end_point - fit_end_point)
    # eps = 2.0
    # if start_dist > eps and end_dist > eps:
    #     start_dist = np.linalg.norm(start_point - fit_end_point)
    #     end_dist = np.linalg.norm(end_point - fit_start_point)
    #     if start_dist > eps and end_dist > eps:
    #         print()
